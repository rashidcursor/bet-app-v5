...
const UNIBET_LIVE_MATCHES_HEADERS = {
  'accept': '*/*',
  'accept-language': 'en-US,en;q=0.9',
...
async function fetchKambiLiveData() {
  try {
    ...
    if (kambiCache.data && kambiCache.lastUpdated &&
      ...
      console.log('📦 [NEXT API] Using cached Kambi data');
    ...
    if (kambiCache.data && kambiCache.lastUpdated) {
      console.warn('⚠️ [NEXT API] Kambi proxy fetch failed, using cached data');
      return kambiCache.data;
    }
    ...
    console.error('❌ [NEXT API] Failed to fetch Kambi live data via proxy:', error.message);
    return null;
  }
}
